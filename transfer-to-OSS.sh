...
do
        ...
        then
                ...
                echo "$DATE_IF Proceeding to transfer file $file in OSS Bucket $bucket_name" >> /reports/transfer-to-oss-logs/transfer-to-oss-v3-$DATE_FILE.log
                /reports/scripts/ossutil64 cp /reports/Reports_Passed/$file oss://ctvi-bucket/ctvi-reports/ >> /reports/transfer-to-oss-logs/transfer-to-oss-v3-$DATE_FILE.log
        else 
                DATE_ELSE=`date +"%Y-%m-%d %H:%M:%S"`
                echo "$DATE_ELSE File $file already exist on OSS Bucket $bucket_name" >> /reports/transfer-to-oss-logs/transfer-to-oss-v3-$DATE_FILE.log
        fi
done
